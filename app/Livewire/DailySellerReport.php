...
class DailySellerReport extends Component implements HasForms, HasTable
{
        ...
        return $table
            ->query(fn () => User::with([
                                    'orderItems' => function (Builder $query) {
                                        $query->whereRelation('order', 'orders.status', 'paid')->whereDate('order_items.created_at', now());
                                    },
                                    'expenseItems' => function (Builder $query) {
                                        $query->whereDate('expense_items.created_at', now());
                                    }, 
                                    'creditSalePayments' => function (Builder $query) {
                                        $query->whereDate('credit_sale_payments.created_at', now());
                                    },
                                ])
