...
class ExpiredProducts extends Component implements HasForms, HasTable
{
    ...
    {
        return $table
            ->query(fn () => Product::whereNotNull('expire_date')
                                ->whereDate('expire_date', '<=', now())
                                ->where('team_id', Filament::getTenant()->id)
                                // ->orderBy('order_items_count', 'desc')
                )
            ->emptyStateHeading('No expired product.')
            ->emptyStateDescription('Once some of your products get expired, you will find them here.')
            ->columns([
