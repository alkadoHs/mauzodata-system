...
class WeeklySellerReport extends Component implements HasForms, HasTable
{
        ...
        return $table
            ->query(fn () => User::with([
                                    'orderItems' => function (Builder $query) {
                                        $query->whereRelation('order', 'orders.status', 'paid')->whereBetween('order_items.created_at', [now()->startOfWeek(), now()->endOfWeek()]);
                                    },
                                    'expenseItems' => function (Builder $query) {
                                        $query->whereBetween('expense_items.created_at', [now()->startOfWeek(), now()->endOfWeek()]);
                                    }, 
                                    'creditSalePayments' => function (Builder $query) {
                                        $query->whereBetween('credit_sale_payments.created_at', [now()->startOfWeek(), now()->endOfWeek()]);
                                    },
                                ])
